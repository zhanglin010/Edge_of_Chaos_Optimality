...
def plot_lyapunov1s(args):
    ...
    lyapunovss = np.array([[lyapunovs[num_repeat][epoch]
                            for epoch in range(args.epochs//1)] for num_repeat in range(args.num_repeats)])
    ax1.errorbar(x[:21], lyapunovss.mean(axis=(0, 2))[:21],
                 yerr=lyapunovss.mean(axis=2).std(axis=0)[:21],
                 fmt='o-', c='C3', ms=3, lw=1.5, capsize=1.3, elinewidth=0.6,
    ...
    # ax2 = ax1.twinx()
    ax2.errorbar(x[:21], val_loss.mean(axis=0)[:21], yerr=val_loss.std(axis=0)[:21], fmt='o-',
                 c='dimgrey', ms=3, lw=1.5, capsize=1.3, elinewidth=0.6, label='Test loss')
    ...
    ax2.set_xticks([])
    ax2.set_xlim([-0.4, 20.4])
    if args.architecture == 'mlp':
        ax2.set_ylim(0.29, 0.505)
    elif args.architecture == 'cnn':
        # CNN5
        ax2.set_ylim(0, 3.25)
    elif args.architecture == 'cnn_dropout':
    ...
    fig.legend(fontsize=19, loc='upper right', bbox_to_anchor=(0.918, 0.935))
    ...
    plt.savefig('results/{}_{}/lyapunov1s_21'.format(args.dir, args.num_repeats),
...
def plot_acc(args):
    ...
    val_acc = np.array([[losses[num_repeat]['val'][epoch][1]
                         for epoch in range(args.epochs//1)] for num_repeat in range(args.num_repeats)])
    ax1.errorbar(x[:21], train_acc.mean(axis=0)[:21], yerr=train_acc.std(axis=0)[:21],
                 fmt='o-', c='darkolivegreen', ms=3.2, lw=1.5, capsize=1.3, elinewidth=0.6, zorder=10,
                 label='Train')
    ax1.errorbar(x[:21], val_acc.mean(axis=0)[:21], yerr=val_acc.std(axis=0)[:21],
                 fmt='o-', c='C5', ms=3.2, lw=1.5, capsize=1.3, elinewidth=0.6, label='Test')
    ...
    if not os.path.exists('results/{}_{}'.format(args.dir, args.num_repeats)):
        os.makedirs('results/{}_{}'.format(args.dir, args.num_repeats))
    plt.savefig('results/{}_{}/acc_loss_1_21'.format(args.dir, args.num_repeats),
                bbox_inches='tight', dpi=300, transparent=True)
        ...
        for indx, epoch in enumerate(np.arange(0, 15)):
            for num_iterations in range(args.num_iterations//2-1, args.num_iterations-1):
                axs[indx//4, indx % 4].plot(length1s_proj[num_repeat, epoch, num_iterations, image_num],
                                            length1s_proj[num_repeat, epoch, num_iterations+1, image_num], 'o', color='C0', ms=4)
...
def plot_poincare_separate(args):
    ...
    length2s_proj = np.array([[length2s_proj[num_repeat][epoch] for epoch in range(
        args.epochs//1)] for num_repeat in range(args.num_repeats)])
    for indx, epoch in enumerate([3, 6, 13]):
        if indx == 0:
            fig, ax1 = plt.subplots(figsize=(1.58, 2), dpi=300)
            ax1.set_xlim(1.2032, 1.2167)
            ax1.set_ylim(1.2032, 1.2167)
            ax1.set_xticks([1.205, 1.215])
            ax1.set_yticks([1.205, 1.215])
            ax1.set_yticklabels([])
            ax1.set_ylabel(r'$\overline{X}_{t+1}$', fontsize=12)
            ms1 = 6
            ms2 = 4
        elif indx == 1:
            fig, ax1 = plt.subplots(figsize=(1.165, 2), dpi=300)
            ax1.set_xticks([1.16, 1.22])
            ax1.set_yticks([1.16, 1.22])
            ax1.set_yticklabels([])
            ms1 = 6
            ms2 = 4
        elif indx == 2:
